...
const getIssuerIcon = (issuer: string) => {
  ...
  if (norm.includes("google") || norm.includes("coursera")) {
    // Elegant shield containing a dynamic key/letter
    return (
  ...
  // Specialist: certificate.dev, Codedamn, coddy.tech - elegant CPU/Node/Star badge
  return (
    <svg className="w-5 h-5 text-[#39FF14]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  return (
    <section id="certifications" className="py-24 border-t border-white/5 relative z-20 overflow-hidden bg-[#050816]">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="space-y-16 text-left">
          {certificationsData.map((group, groupIdx) => (
            <div key={groupIdx} className="space-y-6">
              ...
              >
                <span className={`text-[#39FF14] text-xs font-mono font-bold shrink-0`}>
                  ►
                </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {group.items.map((cert, certIdx) => (
                  <motion.div
                    ...
                    className="group relative flex items-start gap-4 p-5 rounded-lg bg-[#080D1F] border-l-2 transition-all duration-200 hover:bg-[#0D1530] hover:shadow-[0_0_0_1px_rgba(57,255,20,0.15)]"
                    style={{ borderLeftColor: '#39FF14' }}
                  >
                    {/* Icon container */}
                    <div className="p-1.5 rounded bg-white/5 text-[#39FF14] shrink-0 mt-0.5">
        ...
        >
          {/* Label Above */}
          <span className="block font-mono text-[11px] font-bold text-[#39FF14] uppercase tracking-[0.15em]">
            // FORMAL EDUCATION
          </span>
          ...
          >
            ...
            <div className="max-w-3xl space-y-6">
              <div>
                ...
                <p className="text-[#39FF14] font-mono text-xs sm:text-sm font-semibold tracking-wide mt-2">
